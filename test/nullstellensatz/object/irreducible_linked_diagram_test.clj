...
(deftest check-concat-codes
  (let [m-1 [[1 2]]
        m-2 [[1 3] [2 4]]
        m-3 [[1 5] [2 4] [3 6]]
        m-4 [[1 4] [2 6] [3 5]]
        m-5 [[1 3] [2 5] [4 6]]
        ->sorted-concat (comp sort irreducible/concat-codes)]
    (is (match? [[1 3] [2 4]] (->sorted-concat 0 m-1 m-1)))
    (is (match? [[1 5] [2 4] [3 6]] (->sorted-concat 0 m-1 m-2)))
    (is (match? [[1 4] [2 6] [3 5]] (->sorted-concat 0 m-2 m-1)))
    (is (match? [[1 4] [2 5] [3 6]] (->sorted-concat 1 m-2 m-1)))
    (is (match? [[1 3] [2 5] [4 6]] (->sorted-concat 2 m-2 m-1)))
    (is (match? [[1 3] [2 5] [4 7] [6 8]] (->sorted-concat 4 m-5 m-1)))
    (is (match? [[1 8] [2 9] [3 7] [4 6] [5 10]] (->sorted-concat 1 m-2 m-3)))
    (is (match? [[1 8] [2 5] [3 14] [4 6] [7 10] [9 12] [11 13]]
                (->sorted-concat 0 (->sorted-concat 4 m-5 m-1) m-4)))))
